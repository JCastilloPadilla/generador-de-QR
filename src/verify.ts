 ...
 *
 * Se verifica el canvas completo, con logo y formas incluidos, porque es
 ...
 * legibilidad depende del patrón, no de a cuántos píxeles se exporte, así que
 * verificar a 2048 px solo costaría tiempo.
 */
const VERIFY_SIZE = 512;

export type VerifyState = 'ok' | 'ilegible' | 'difiere';
...
export interface Verifier {
  (matrix: QrMatrix, style: RenderStyle, expected: string): Verification;
}

/**
 * Deduce por qué no se lee. No es una certeza, y el texto lo dice: son las tres
...
function decode(image: ImageData, style: RenderStyle): { data: string } | null {
  try {
...
export function createVerifier(): Verifier {
  ...
  return (matrix, style, expected) => {
    drawToCanvas(canvas, matrix, VERIFY_SIZE, style);

    const ctx = canvas.getContext('2d');
    if (!ctx) return { state: 'ilegible', decoded: null, reason: null };

    const image = ctx.getImageData(0, 0, canvas.width, canvas.height);
    const result = decode(image, style);
